...
function plugin(options) {
  ...
  return through.obj(function collectRevs(file, enc, cb) {
    ...
    if (file.revOrigPath) {
      renames.push({
        revHash: file.revHash || options.hash,
    ...
    function replaceContents() {
      ...
      cache.forEach(function replaceInFile(file) {
        ...
        renames.forEach(function replaceOnce(rename) {
          ...
          if (options.hash) {
            var fn = unreved + '?v=' + rename.revHash;
            if (contents.indexOf(fn) < 0) {
              contents = contents.split(unreved).join(fn);
            }
          } else {
        ...
        if (options.hash && file.revOrigPath) {
